...
def scan_trending_stocks(held_symbols: set[str] | None = None, cycle_num: int = 0) -> list[str]:
    ...
    for r in results:
        ...
        if r["sym"] not in current and len(current) < MAX_WATCHLIST_SIZE:
            ...
            logger.info(f"  [SCAN] + {r['sym']}: ${r['price']:.2f} ({r['chg']:+.1f}%, vol:{r['vol_ratio']:.1f}x)")
...
def run_trading_cycle(
    ...
    ist = now_ist()
    logger.info(f"\n{'='*60}")
    logger.info(f"  CYCLE #{cycle_num} | {ist.strftime('%Y-%m-%d %H:%M:%S')} ET")
    logger.info(f"{'='*60}")
    ...
    if trader.portfolio.positions:
        for sym, pos in trader.portfolio.positions.items():
            ...
            logger.info(f"    {sym:20s} {pos.quantity}x @ ${pos.avg_price:.2f} -> ${current:.2f}  {pnl_str} ({pnl_pct:+.1f}%)")
        ...
        try:
            if not force and not is_market_open():
                ...
                if wait:
                    ...
                    ist = now_ist()
                    logger.info(f"  [{ist.strftime('%H:%M ET')}] Market closed. Opens in {hours:.1f} hours. Waiting...")
